...
abstract class Grid extends \Nette\Application\UI\Control
{
	...
	/** @var array */
	protected $perPageValues = array(10 => 10, 20 => 20, 50 => 50, 100 => 100);
		...
		try{
			...
			foreach($this->filter as $name => $value){
				if(!$this->columnExists($name)){
					throw new UnknownColumnException("No existing column $name");

				}
				if(!$this['columns-'.$name]->hasFilter()){
					throw new UnknownFilterException("No existing filter for column $name");
				}
	...
	{
		return new GridPaginator($this->translator);
	}
	...
	protected function createComponentGridForm()
	{
		...
		$form[$this->name]['perPage']->addSelect("perPage","Items per page:", $this->perPageValues)
			->getControlPrototype()
	...
	public function actionFormSubmitted($values, $gridName)
	{
		try{
			...
			$subGrid = ($gridName == $this->name) ? FALSE : TRUE;
			if(!count($rows)){
				throw new NoRowSelectedException("There was no record selected");
			}
			if($subGrid){
			...
			if($subGrid){
				$this[$gridName]->flashMessage("There was no record selected","grid-error");
			}else{
				$this->flashMessage("There was no record selected","grid-error");
			}
			$this->redirect("this");
		}
	}
