...
Nette.validators = {
        ...
        range: function (elem, arg, val) {
			if ($(elem).hasClass('date')) {
				var res = val.split(".");
				var value = null;;
				if (res.length === 3 && res[2].length === 4 && res[1].length === 2 && res[0].length === 2) {
					value = res[2] + '-' + res[1] + '-' + res[0];
				}
				var dateFormat = $.datepicker.W3C;
				var date = (value ? $.datepicker.parseDate(dateFormat, value) : null);
				var arg0date = Nette.isArray(arg) && arg[0] !== null ? $.datepicker.parseDate(dateFormat, arg[0].date) : null;
				var arg1date = Nette.isArray(arg) && arg[1] !== null ? $.datepicker.parseDate(dateFormat, arg[1].date) : null;

				return Nette.isArray(arg) ? ((arg0date === null || date >= arg0date) && (arg1date === null || date <= arg1date)) : null;
			}
